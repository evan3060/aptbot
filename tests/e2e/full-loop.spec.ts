...
describe('E2E: Full agent loop', () => {
  ...
  it('#8 CLI commands: all 14 builtin commands available and executable (I16)', async () => {
    const registry = createCommandRegistry();
    const commands = registry.list();
    expect(commands.length).toBe(14);
    const names = commands.map((c) => c.name).sort();
    expect(names).toEqual(['agent', 'clear', 'continue', 'exit', 'feedback', 'help', 'label', 'model', 'new', 'resume', 'session', 'session.reset', 'sessions', 'skill']);
