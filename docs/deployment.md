...
在 `config/aptbot.json` 顶层加 `"landingPage": true` 等配置（见第 5 节示例）。
...
## 10.6 v0.2.3 知识体系 + 双语部署

v0.2.3 新增知识体系（`/learn` 列表页 + `/learn/:slug` 文章页 + `/feedback` 反馈页）+ 双语文章（中/英 `?lang=en`）+ i18n 界面翻译。

### 启用知识体系

在 `config/aptbot.json` 同时设置 `landingPage: true` 和 `learnPage: true`：
...
| 路径 | 说明 |
|------|------|
| `/learn` | 知识列表页（展示所有文章，按 Track/Chapter 分组） |
| `/learn/:slug` | 文章详情页（?lang=en 切换英文版） |
| `/learn?lang=en` | 英文版列表页 |
| `/feedback` | 通用反馈页 |
