...
- 背景：**始终** `rgba(255, 255, 255, 0.85)` 半透明白 + `backdrop-filter: blur(8px)`（验收修订：原设计"滚动前透明"导致下层文字透出叠加，改为始终遮挡）
- `transition: border-color 300ms ease-in-out, box-shadow 300ms ease-in-out`（验收修订：原 `background-color` transition 不再需要）
- `.scrolled` 仅加 `border-bottom: 1px solid var(--border)` + 轻微 `box-shadow` 做视觉分层（不再是遮挡开关）
...
## 验收增补（v0.2.1 用户验收期间发现并修复的问题）

本章节记录在用户验收期间发现、并在 v0.2.1 分支内修复的问题。原 plan 的 7 个 task 未覆盖这些移动端与滚动相关的视觉问题，但属于本版本落地页/demo 页体验的必要修复。

### 增补 1：chat-page.ts 移动端适配（commit a606d59 + fe93d25）

**问题：** 390 视口下 sidebar 固定 260px 导致 main 仅剩 130px，对话区被严重挤压；横向滚动条出现（scrollWidth 537 vs clientWidth 390）。

**修复：**
- `@media (max-width: 768px)` 媒体查询：sidebar 转为 `position: fixed` + `transform: translateX(-100%)` 抽屉化
- Header 增加 `#sidebar-toggle` hamburger 按钮（桌面 `display: none`），v2 改用 SVG 三横线图标
- `#sidebar-backdrop` 遮罩（v2 加 `backdrop-filter: blur(2px)`）点击关闭 sidebar
...
**测试：** `tests/access/chat-page-mobile.spec.ts`（17 项，含 SVG 图标 / box-shadow / cubic-bezier / backdrop-filter / session 自动收起断言）

### 增补 2：landing-page.ts 移动端字体与椭圆框精致化（commit 4af1df5）

**问题：** 390 视口下 hero CTA pill 24px 字体 + 12px 36px padding → 220×51 椭圆框过大；hero h1 48px 与 subtitle 20px 落差大；section h2 / eval-value / card 文字未在移动端缩放；nav-demo-btn / nav-lang 行高叠加导致椭圆框高度异常（68px / 39px）。

**修复（`@media (max-width: 767px)` 块内）：**
- body：20px/25px → 16px/22px（基础字号协调）
- nav-demo-btn：14px / 6px 14px padding / line-height 1.2 → 高度 31px
- nav-lang：12px / 4px 10px / line-height 1.2 → 高度 24px
- hero h1：48px/48px → 36px/40px（letter-spacing -1.5px）
- hero subtitle：20px/25px → 16px/22px
- btn-pill：24px / 12px 36px → 16px / 10px 24px（147×44 精致椭圆）
- section h2-lg：48px → 28px / h2-md：36px → 24px / h2-sm：24px → 16px
- eval-value：48px → 28px
- card h3：20px → 17px / card-desc：20px/25px → 15px/21px
- data-bar：移动端 2 列网格
- 页面总高度 6560px → 4730px（更紧凑）

**测试：** `tests/access/landing-page-mobile.spec.ts`（14 项，含椭圆框 padding / line-height / 字号上限断言）

### 增补 3：nav 滚动文字叠加修复（commit e37c226）

**问题：** nav 是 `position: fixed`（设计内行为），但默认 `background-color: rgba(0, 0, 0, 0)` 透明，`.scrolled` 的 `background-color: var(--surface-translucent)` 在 computed style 中未生效，移动端 IntersectionObserver 触发时机太晚。结果：滚动时下层 hero/section 文字透出，与 nav 文字叠加。

**修复：** 让 nav 始终有遮挡，不再依赖 `.scrolled` 切换遮挡开关。
- `#nav` 默认：`background-color: rgba(255, 255, 255, 0.85)` + `backdrop-filter: blur(8px)`
- `#nav` 默认：`border-bottom: 1px solid transparent`（占位）
- `transition` 改为 `border-color 300ms, box-shadow 300ms`（不再 transition background-color）
- `.scrolled` 仅加 `border-bottom: 1px solid var(--border)` + `box-shadow: 0 1px 6px rgba(0,0,0,0.04)` 做视觉分层

**测试：** `tests/access/landing-page-nav-scroll.spec.ts`（5 项，含默认非透明背景 / backdrop-filter / .scrolled 不再切换背景断言）

### 增补后测试总量

- 原 plan 7 task 产出：651 项测试
- 增补 1：+17 项（chat-page-mobile）
- 增补 2：+14 项（landing-page-mobile）
- 增补 3：+5 项（landing-page-nav-scroll）
- 增补后总计：687 项测试全绿
